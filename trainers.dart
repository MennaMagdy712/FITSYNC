...
const List<String> kAllSpecializations = [
  ...
  'Pilates', 'Cross Fit', 'Zumba',
  'Martial Arts', 'Dance Fitness',
];
...
class _TrainersPageState extends State<TrainersPage> {
  final List<Trainer> _trainers = [
    Trainer(name: 'Youssef Ahmed', email: 'Youssef@gmail.com', phone: '01095121008', initials: 'YA', dob: '7/4/2005', address: '3-Alansari street - Sinbellwin', specializations: ['Martial Arts']),
  ...
  void _openAdd() async {
    final result = await Navigator.push(
        context, MaterialPageRoute(builder: (_) => AddTrainerPage()));
    if (result is Trainer) setState(() => _trainers.add(result));
  }

  void _openDetails(Trainer t) async {
    final result = await Navigator.push(
        context, MaterialPageRoute(builder: (_) => TrainerDetailsPage(trainer: t)));
    if (result == 'deleted') {
      setState(() => _trainers.remove(t));
    } else if (result is Trainer) {
      setState(() {});
    }
  }
      ...
      builder: (_) => Padding(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _menuItem(Icons.edit_outlined, 'Edit Details', Colors.white, () async {
              Navigator.pop(context);
              final result = await Navigator.push(context,
                  MaterialPageRoute(builder: (_) => EditTrainerPage(trainer: t)));
              if (result is Trainer) setState(() {});
            }),
            _menuItem(Icons.delete_outline, 'Delete', Colors.red, () async {
              Navigator.pop(context);
              final result = await Navigator.push(context,
                  MaterialPageRoute(builder: (_) => DeleteTrainerPage(trainer: t)));
              if (result == 'deleted') setState(() => _trainers.remove(t));
            }),
      ...
      body: SafeArea(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: const Row(children: [
            ...
            Expanded(flex: 3, child: Text('Specialization', style: _hStyle)),
        ...
        // List
        Expanded(child: _trainers.isEmpty
          ? const Center(child: Text('No trainers yet', style: TextStyle(color: kGrey)))
          : ListView.separated(
...
const _hStyle = TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Color(0xFF555555));

class _TrainerRow extends StatelessWidget {
  final Trainer trainer;
  final VoidCallback onTap, onMenu;
  _TrainerRow({required this.trainer, required this.onTap, required this.onMenu});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
        child: Row(children: [
          Expanded(flex: 3, child: Text(trainer.name, style: const TextStyle(fontSize: 12, color: kBlack))),
          Expanded(flex: 3, child: Text(trainer.email,
              style: const TextStyle(fontSize: 11, color: kOrange, decoration: TextDecoration.underline))),
          Expanded(flex: 3, child: Text(trainer.phone, style: const TextStyle(fontSize: 11, color: kBlack))),
          Expanded(flex: 3, child: Text(
            trainer.specializations.isEmpty ? '-' : trainer.specializations.join(', '),
...
class TrainerDetailsPage extends StatelessWidget {
  final Trainer trainer;
  TrainerDetailsPage({required this.trainer});
        ...
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(children: [
              ...
              child: Column(children: [
                Container(
                  ...
                  child: Text(trainer.initials,
                      style: const TextStyle(color: kOrange, fontSize: 20, fontWeight: FontWeight.w800)),
                ),
                const SizedBox(height: 8),
                Text(trainer.name,
                    style: const TextStyle(color: kBlack, fontSize: 16, fontWeight: FontWeight.w800)),
                ...
                if (trainer.specializations.isNotEmpty)
                  Container(
                    ...
                    child: Text(trainer.specializations.join(', '),
            ...
            Container(
              ...
              child: Column(children: [
                _detailRow(Icons.email_outlined, 'Email', trainer.email, Colors.orange),
                const Divider(height: 1, color: kBorder),
                _detailRow(Icons.phone_outlined, 'Phone', trainer.phone, Colors.green),
                const Divider(height: 1, color: kBorder),
                _detailRow(Icons.calendar_today_outlined, 'Date Of Birth',
                    trainer.dob.isEmpty ? '-' : trainer.dob, Colors.blue),
                const Divider(height: 1, color: kBorder),
                _detailRow(Icons.location_on_outlined, 'Address',
                    trainer.address.isEmpty ? '-' : trainer.address, Colors.red),
              ]),
            ),
            ...
            SizedBox(width: double.infinity, child: ElevatedButton(
              ...
              child: const Text('Back To List', style: TextStyle(fontWeight: FontWeight.w700)),
            )),
          ]),
        )),
      ])),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Text(value, style: const TextStyle(fontSize: 12, color: kBlack)),
          ),
        ])),
      ]),
    );
...
class _AddTrainerPageState extends State<AddTrainerPage> {
  ...
  @override
  void dispose() {
    for (final c in [_name, _email, _phone, _dob, _building, _street, _city]) c.dispose();
    super.dispose();
  }
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ...kAllSpecializations.map((s) => CheckboxListTile(
              value: _selected.contains(s),
              onChanged: (v) => setState(() => v! ? _selected.add(s) : _selected.remove(s)),
              title: Text(s, style: const TextStyle(fontSize: 13, color: kBlack)),
              activeColor: kOrange,
...
class _EditTrainerPageState extends State<EditTrainerPage> {
  late TextEditingController _name, _email, _building, _street, _city;
  ...
  @override
  void dispose() {
    for (final c in [_name, _email, _building, _street, _city]) c.dispose();
    super.dispose();
  }

  void _save() {
    widget.trainer.name    = _name.text.trim();
    widget.trainer.email   = _email.text.trim();
    widget.trainer.address = _building.text.trim();
    widget.trainer.specializations = _selected.toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: SingleChildScrollView(
          padding: const EdgeInsets.all(20),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Edit Trainer',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: kOrange)),
            Text('Update trainer information',
                style: const TextStyle(fontSize: 12, color: kGrey)),
            const SizedBox(height: 8),
            Text('Editing info for:', style: const TextStyle(fontSize: 11, color: kGrey)),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                  ...
                  ElevatedButton(
                    ...
                    child: const Text('Update Trainer', style: TextStyle(fontWeight: FontWeight.w700)),
                  ),
                ]),
              ]),
            ),
...
// DELETE TRAINER PAGE
// ══════════════════════════════════════════════════════════════════════════════
class DeleteTrainerPage extends StatelessWidget {
  final Trainer trainer;
  DeleteTrainerPage({required this.trainer});
  ...
  return Container(
    ...
    child: Row(children: [
      ...
      Container(
        ...
        child: const Text('Trainer', style: TextStyle(color: kBlack, fontSize: 11, fontWeight: FontWeight.w700)),
      ),
    ]),
  );
}
